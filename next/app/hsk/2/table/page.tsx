...
export default function Hsk2TablePage() {
  const searchParams = useSearchParams();
  const [level, setLevel] = useState("1");
  const [limit, setLimit] = useState(0);
  const [hskWords, setHskWords] = useState<OldHskWordType[] | null>(null);
  ...
  async function loadLexicon(lvl: string, lmt: number) {
    ...
    try {
      const data = await apiGetReq(`/api/lexicon?hsk_level=${lvl}&limit=${lmt}`);
    ...
    <>
      <div className='row'>
        <div className='col-sm-3'>
          <TableCard level={level} setLevel={setLevel} isOldHsk={true} isForTests={false} />
        </div>

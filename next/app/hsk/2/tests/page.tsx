...
export default function NewHskTests({}) {
  ...
  async function loadRandWords(lvl: string) {
    setHskWords(null);
    try {
      const data = await apiGetReq(`/api/lexicon/all?hsk_level=${lvl}`);
      setHskWords(data.map((x: OldHskWordType) => new TestWord(x)));
    } catch (err) {
      console.log(err);
    }
  }
  ...
  const pronounce = (id: number, lvl: string) => {
    ...
    new Audio(`${constUrls.myAudioURL}hsk${lvl}/${id - 1 - firstIdPerLvl[lvl]}.mp3`).play();
  };
  ...
  return (
    <div className='row'>
      <div className='col-sm-3'>
        <TableCard level={level} setLevel={setLevel} isOldHsk={true} isForTests={true} />
      </div>

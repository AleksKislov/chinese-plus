...
function start() {
  ...
  for (let i = STARTING_LINE; i < lines.length; i++) {
    ...
    // if (typeof dictionarysimplified[multiplearray[0].simplified] != "undefined") {
    ...
    if (typeof dictionarytraditional[multiplearray[0].traditional] != "undefined") {
      var newMultipleArray = dictionarytraditional[multiplearray[0].traditional];
      for (var j = 0; j < multiplearray.length; j++) {
        newMultipleArray.push(multiplearray[j]);
      }
  ...
  // console.log(Object.keys(dictionarytraditional).length);
  // console.log(Object.keys(dictionarysimplified).length);
...
async function definitionLookup(word, scripttype) {
  if (scripttype == null) {
    if (determineIfSimplified(word)) return dictionarysimplified[word];

    if (!determineIfSimplified(word)) {
      // console.log(dictionarytraditional[word]);
...
function determineIfSimplified(character) {
  if ("undefined" != typeof dictionarysimplified[character]) return true;
  if ("undefined" != typeof dictionarytraditional[character]) return false;
}
